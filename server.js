...
app.post("/api/login", (req,res)=>{
  ...
  const s=db.prepare("SELECT * FROM students WHERE (name=? OR roll_number=?) AND password=?").get(login,login,hash(password));
  if(!s) return res.status(401).json({error:"Invalid student name/roll number or password"});
...
app.put("/api/my-profile",studentRequired,(req,res)=>{
  ...
  try {
    ...
    if(req.body.new_password){
      ...
      if(hash(oldPassword)!==s.password) return res.status(401).json({error:"Current password is incorrect"});
      if(String(req.body.new_password).length<6) return res.status(400).json({error:"New password must be at least 6 characters"});
      db.prepare("UPDATE students SET password=? WHERE id=?").run(hash(req.body.new_password),s.id);
    }
...
app.put("/api/faculty/password",facultyRequired,(req,res)=>{
  const f=facultyFromReq(req);
  const oldPassword=String(req.body.oldPassword||"");
  const newPassword=String(req.body.newPassword||"").trim();
  if(!oldPassword || !newPassword) return res.status(400).json({error:"Current and new password are required"});
  if(newPassword.length < 6) return res.status(400).json({error:"New password must be at least 6 characters"});
  if(hash(oldPassword)!==f.password) return res.status(401).json({error:"Current password is incorrect"});
  db.prepare("UPDATE faculty SET password=? WHERE id=?").run(hash(newPassword),f.id);
  res.json({ok:true});
});

app.get("/api/class-overview",studentRequired,(req,res)=>{
  const rows=db.prepare(`SELECT s.id,s.name,
    SUM(CASE WHEN a.status='Present' THEN 1 ELSE 0 END) present,
    SUM(CASE WHEN a.status='Absent' THEN 1 ELSE 0 END) absent, COUNT(a.id) total
    FROM students s LEFT JOIN attendance a ON a.student_id=s.id GROUP BY s.id ORDER BY s.name COLLATE NOCASE`).all()
    .map(x=>{const present=Number(x.present||0),absent=Number(x.absent||0),total=Number(x.total||0);return {...x,present,absent,total,percentage:total?Number((present/total*100).toFixed(2)):0};});
  res.json({students:rows});
});
